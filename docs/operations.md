...
| `scripts/make_walls.py` | 每周（Sync Reuploads 最后一步）或手动 | 合成页眉横幅的封面墙 `static/wall.webp`（投稿，按月取样 80 张）与 `static/wall-live.webp`（本月之前最近 64 场直播）；不联网，内容没变不写文件，所以图每月最多变一次 |
